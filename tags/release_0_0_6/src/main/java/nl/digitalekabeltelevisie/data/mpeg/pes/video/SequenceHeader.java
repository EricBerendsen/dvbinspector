...
public class SequenceHeader extends VideoMPEG2Section {
	...
	public SequenceHeader(final byte[] data, final int offset) {
		...
		//		horizontal_size_value = Utils.getInt(data, offset+1, 2, 0xFFF0)>>4;
		//		vertical_size_value = Utils.getInt(data, offset+2, 2, 0x0FFF);
		//		aspect_ratio_information = Utils.getInt(data, offset+4, 1, 0xF0)>>4;
		//		frame_rate_code = Utils.getInt(data, offset+4, 1, 0x0F);
		//		bit_rate_value = Utils.getInt(data, offset+5, 3, 0xFFFFC0)>>6;
		//		marker_bit = Utils.getInt(data, offset+7, 1, 0x20)>>5;
	}
