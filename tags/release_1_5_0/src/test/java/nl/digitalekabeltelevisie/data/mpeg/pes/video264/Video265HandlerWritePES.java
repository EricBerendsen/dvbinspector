 ...
 *
 */
public class Video265HandlerWritePES {

	/**
	 * 
	 */
	private static TransportStream transportStream;
	private static final Integer HEVC_PID = 257;
	...
	public static void setUp() throws NotAnMPEGFileException, IOException, URISyntaxException{
		...
		// spaces in dirname...
		final File ts =  new File("H:\\ts\\UHD_test_streams\\HotBird_130E_10930H_UHD_test_2015-02-23_.001");
		transportStream = new TransportStream(ts);
		transportStream.parseStream();

		final Map<Integer, GeneralPesHandler> map = new HashMap<>();

		final PID p1= transportStream.getPID(HEVC_PID);
		map.put(HEVC_PID, p1.getPesHandler());
	...
	public void testNPO1() {
		...
		final PID npo1 = transportStream.getPID(HEVC_PID);
		...
		final H265Handler video14496Handler = (H265Handler) pesHandler;

		final List<PesPacketData> pesPackets = video14496Handler.getPesPackets();

		final String FILE_NAME = "d:\\8\\hevcpes.pes";
		...
		try {
			...
			for (final PesPacketData packet : pesPackets) {
				...
				final byte[] aBytes = Arrays.copyOfRange(data, 0, start+len);
