...
public abstract class AbstractPSITabel implements TreeNode{
	...
	protected void updateSectionVersion(final TableSection newSection, final TableSection section) {

		TableSection last = section;

		//TODO look into currentNext, now only looks for version.
		while(last.getNextVersion()!=null){
			if((last.getVersion()==newSection.getVersion())&&(last.getCurrentNext()==newSection.getCurrentNext())){
				// this version already exists, update the count and last pakcet
		...
		}else{  // already have an instance if this section, just update the stats on the existing section
			last.setLast_packet_no(newSection.getPacket_no());
			last.setOccurrence_count(last.getOccurrence_count()+1);
