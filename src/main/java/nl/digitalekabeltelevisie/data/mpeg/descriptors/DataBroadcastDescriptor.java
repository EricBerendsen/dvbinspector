...
public class DataBroadcastDescriptor extends Descriptor {
	...
	public DataBroadcastDescriptor(byte[] b, TableSection parent) {
		super(b, parent);
		dataBroadcastId = Utils.getInt(b, 2, 2, Utils.MASK_16BITS);
		componentTag = Utils.getInt(b, 4, 1, Utils.MASK_8BITS);
		selectorLength = Utils.getInt(b, 5, 1, Utils.MASK_8BITS);
		selectorByte = copyOfRange(b, 6, 6 + selectorLength);
		if (dataBroadcastId == 0x0005) { // en 301192 7.2.1
			MAC_address_range = Utils.getInt(b, 6, 1, 0xE0) >>> 5;
			MAC_IP_mapping_flag = Utils.getInt(b, 6, 1, 0x10) >>> 4;
			alignment_indicator = Utils.getInt(b, 6, 1, 0x08) >>> 3;
			max_sections_per_datagram = Utils.getInt(b, 7, 1, Utils.MASK_8BITS);
		}
		iso639LanguageCode = getISO8859_1String(b, 6 + selectorLength, 3);
		textLength = Utils.getInt(b, 9 + selectorLength, 1, Utils.MASK_8BITS);
		text = copyOfRange(b, 10 + selectorLength, 10 + selectorLength + textLength);
	}
	...
	public KVP getJTreeNode(int modus){
		...
		if(dataBroadcastId==0x0005){ //en 301192 7.2.1
			t.add(new KVP("MAC_address_range",MAC_address_range,getvalidMACaddressBytesString(MAC_address_range)));
			t.add(new KVP("MAC_IP_mapping_flag",MAC_IP_mapping_flag,MAC_IP_mapping_flag==1?"uses IP to MAC mapping as described in RFC 1112 and RFC 2464":"mapping not defined"));
			t.add(new KVP("alignment_indicator",alignment_indicator,alignment_indicator==1?"alignment in bits: 32":"alignment in bits: 8 (default)"));
			t.add(new KVP("max_sections_per_datagram",max_sections_per_datagram));
		}else{
			t.add(new KVP("selector_bytes",selectorByte));
		}
		t.add(new KVP("ISO_639_language_code",iso639LanguageCode));
		t.add(new KVP("text_length",textLength));
		t.add(new KVP("text_char",text));

		return t;
	}

	public static String getvalidMACaddressBytesString(int adresReange) {
		return switch (adresReange) {
		case 0x00 -> "reserved";
		case 0x01 -> "valid MAC_address bytes: 6";
		case 0x02 -> "valid MAC_address bytes: 6,5";
		case 0x03 -> "valid MAC_address bytes: 6,5,4";
		case 0x04 -> "valid MAC_address bytes: 6,5,4,3";
		case 0x05 -> "valid MAC_address bytes: 6,5,4,3,2";
		case 0x06 -> "valid MAC_address bytes: 6,5,4,3,2,1";
		default -> "reserved";
		};
	}
