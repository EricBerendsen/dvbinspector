...
public class PATsection extends TableSection {
	...
	public DefaultMutableTreeNode getJTreeNode(final int modus){
		...
		t.add(new DefaultMutableTreeNode(new KVP("programs_length",noPrograms(),null)));

		Utils.addListJTree(t,programs,modus,"programs");

		//		DefaultMutableTreeNode programsListNode = new DefaultMutableTreeNode(new KVP("programs");
		//		for (Iterator<Program> iter = programs.iterator(); iter.hasNext();) {
		//			programsListNode.add(iter.next().getJTreeNode());
		//		}
		//		t.add(programsListNode);
		//
		return t;
	}
