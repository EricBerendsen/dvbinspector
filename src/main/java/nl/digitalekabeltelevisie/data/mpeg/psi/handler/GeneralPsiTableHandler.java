...
import nl.digitalekabeltelevisie.data.mpeg.psi.atsc.ATSCETTsection;
...
public class GeneralPsiTableHandler extends GeneralPidHandler {
	...
	private void handleATSCETT(final TableSection section) {
		if (atsc == null) {
			atsc = new ATSCTables(getTransportStream().getPsi());
		}
		ATSCETTsection s = new ATSCETTsection(section.getRaw_data(), pid);
		copyMetaData(section, s);
		atsc.update(s);
	}
