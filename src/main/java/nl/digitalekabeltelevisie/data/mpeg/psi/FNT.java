...
public class FNT extends AbstractPSITabel{
	...
	private Map<Integer, Map<Integer, FNTsection []>> networks = new HashMap<>();


	public FNT(final PSI parent){
		super(parent);
	}
	...
	public void update(final FNTsection section){
		...
		Map<Integer, FNTsection[]> pidNetwork = networks.computeIfAbsent(pid, k -> new HashMap<>());
		FNTsection[] sections = pidNetwork.computeIfAbsent(key, k -> new FNTsection[section.getSectionLastNumber()+1]);
	...
	public DefaultMutableTreeNode getJTreeNode(final int modus) {
		...
		final DefaultMutableTreeNode t = new DefaultMutableTreeNode( new KVP("FNT (M7 Fastscan Network Table)"));
		for (Integer pid : new TreeSet<Integer>(networks.keySet())) {
			Map<Integer, FNTsection[]> operatorsInPid = networks.get(pid);
			...
			for (Integer operator_network_id : new TreeSet<Integer>(operatorsInPid.keySet())) {
				...
				final FNTsection [] sections = operatorsInPid.get(operator_network_id);
				for (final FNTsection tsection : sections) {
					if(tsection!= null){
