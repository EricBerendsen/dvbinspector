...
public class MessageElementPI extends MessageElement {

	public MessageElementPI(final byte[] data, final int start) {
		super(data, start);
	}
	...
	public DefaultMutableTreeNode getJTreeNode(final int modus) {
		final DefaultMutableTreeNode messageElement = new DefaultMutableTreeNode(new KVP("Message Element PI"));
		...
		messageElement.add(new DefaultMutableTreeNode(new KVP("Programme Identification Code",getPICode(),null)));

		return messageElement;
	}

	protected int getPICode() {
		return Utils.getInt(data, st+3, 2, Utils.MASK_16BITS);
	}
}
